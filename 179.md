...
Sending `1 wei` of Ether from L2 to OptimismPortal contract address is possible, which sends it backwards to the L1StandardBridge address on L2. As this is double transfer it provides a protocol griefing surface as the attacker costs are tiny in this case.

Sending from L1 to L2ToL1MessagePasser address is possible as well, but is more costly.

## Vulnerability Detail

Destinations of ETH transfers are controlled to be not bridge or messenger (both L1 and L2, this code is universal), while Portal and Passer are allowed.

Notice that messenger level _isUnsafeTarget() check doesn't stop this as the target in the call is the corresponding bridge contract, which is usual workflow and is allowed. 

## Impact

Attacker can DOS the system by initiating vast amount of such transfers from different addresses, with each of them syphoning protocol resources by spending L1 gas. As the attack can be spread wide in time the surge pricing L1 gas periods can be included, i.e. an attacker can do moderate, but periodical transfers and statistically they will catch costly L1 gas and make average cost big enough this way.
